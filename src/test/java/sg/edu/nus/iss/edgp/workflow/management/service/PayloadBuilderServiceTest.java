...
class PayloadBuilderServiceTest {
    ...
    @Test
    @DisplayName("Happy path: wraps fields under data_entry, strips reserved keys (keeps organization_id per source)")
    void buildDataQualityPayLoad_happyPath_alignWithSource() throws Exception {
        ...
        // organization_id is KEPT per current source code
        assertEquals("org-1", data.get("organization_id"));

        // removed per source
        for (String k : List.of(
                "created_date", "domain_name", "failed_validations", "file_id",
                "final_status", "rule_status", "dataquality_status",
                // "organization_id",  <-- NOT removed in source
        ...
        assertEquals("org-1", workflowStatus.get("organization_id"));
    }

    @Test
    @DisplayName("Missing rule fields -> keys present with nulls; data contains remaining workflow fields")
    void buildDataQualityPayLoad_missingRuleFields_setsNulls_alignWithSource() throws Exception {
        Map<String, Object> ruleResponse = new HashMap<>(); // missing all 4 keys

        Map<String, Object> workflowStatus = new HashMap<>();
        workflowStatus.put("records_processed", 10);
        workflowStatus.put("organization_id", "org-2"); // kept per source
        ...
        assertEquals("org-2", data.get("organization_id"));
    }
}
